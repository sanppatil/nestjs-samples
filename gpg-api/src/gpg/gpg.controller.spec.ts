...
export class GpgController {
  ...
  @Post('encrypt-blob')
  async encryptBlob(@Body() body: { sourceBlob: string }) {
    const { sourceBlob } = body;
    await this.gpgService.encryptBlob(sourceBlob);
  ...
  @Post('decrypt-blob')
  async decryptBlob(@Body() body: { sourceBlob: string }) {
    const { sourceBlob } = body;
    await this.gpgService.decryptBlob(sourceBlob);
